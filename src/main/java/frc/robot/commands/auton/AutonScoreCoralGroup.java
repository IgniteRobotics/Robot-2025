...
public class AutonScoreCoralGroup extends ParallelCommandGroup{
  ...
  public SequentialCommandGroup createCommand(){
    return new InstantCommand(() -> CoralState.getInstance().setCoralTarget(m_Position))
        .andThen(new DriveToPoseNoProfile(m_swerveDrivetrain, () -> DriveState.getInstance().getTargetPose2d(), null, null, false).withTimeout(Preferences.autonAlignTimeout.getValue())
          .deadlineFor(new FindReefTarget(m_PhotonCameraWrapper, () -> AllianceState.getInstance().getCustomReefTags(), () -> CoralState.getInstance().pickReefCamera())))
